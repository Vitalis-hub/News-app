...
def manager_perms_add(request):
    ...
    if request.method == 'POST':
        ...
        if len(Permission.objects.filter(codename=cname)) == 0:
            ...
            error = "This codename has been used before"
            return render(request, "back/error.html", {'error': error})
...
def users_perms(request, pk):
    ...
    return render(request, 'back/users_perms.html', {'uperms':uperms, 'pk': pk, 'perms':perms, 'allperms':allperms})
...
def groups_perms_del(request, gname, name):
    ...
    group = Group.objects.get(name=gname)
    perms = Permission.object.get(name=name)
